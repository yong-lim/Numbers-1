...
function preloadImgs() {
	var srcStrArray = [
		...
		'playagain2.jpg',
		'play2.jpg',
		'cartoon-sun-light.jpg',
		'chick_baby_cute_easter_blue.png',
		'brown-bird.jpg',
		'pink-silly-bird.jpg',
		'branch-bird.png',
		'yellow-birds.jpg'
...
function keyBoardFn(event) {
	...
	for (
		char1 = gs.smallestIdLetter;
		char1.charCodeAt(0) <= gs.largestIdLetter.charCodeAt(0);
		char1 = String.fromCharCode(char1.charCodeAt(0) + 1), char2 = String.fromCharCode(char2.charCodeAt(0) + 1)
	) {
	...
	if (!gs.AreCardsSelectable && !gs.isGameStarted) {
		...
		if (card.isOnBoardBool && !card.isTurnedBool && gs.AreCardsSelectable) {
			...
			if (gs.cardSelected1 == null) {
				...
				gs.AreCardsSelectable = false;
				//If cards match
				if (card.gameImageStr.split('.')[0] === gs.cardSelected1.gameImageStr.split('.')[0]) {
					//displayMsgToGameConsole('You found a pair! ' + card.gameImageStr.split('.')[0]);
...
function setGameBg() {
	document.getElementById('grid-container').style.backgroundImage =
		"url('" + gs.pathToImgsStr + gs.gameBgSrcArray[gs.gameBgNb] + "')";
...
function disableControl(idStr) {
	if (
		idStr == undefined ||
		!(idStr === 'play' || idStr === 'playagain' || idStr === 'stop') ||
		typeof idStr != 'string'
	) {
		throw 'Invalid Argument';
	}
...
function Card(idLetterStr) {
	...
	this.showGameImg = function() {
		...
		document.getElementById(this.getGridItemId()).innerHTML =
			'<img id="' + this.idLetterStr + '" src="images/' + this.gameImageStr + '"></img>';
	...
	this.showTopImg = function() {
		if (this.isOnBoardBool) {
			document.getElementById(this.getGridItemId()).innerHTML =
				'<img class="letter" id="' + this.idLetterStr + '" src="images/' + this.idLetterStr + '.png">';
			document.getElementById(this.getGridItemId()).style.backgroundColor = 'whitesmoke';
			document.getElementById(this.getGridItemId()).style.backgroundImage =
				"url('" + gs.pathToImgsStr + gs.topBgStrForAllCards + "')";
			this.isTurnedBool = false;
		}
	};
...
function isGridItemIdValid(idStr) {
	...
	return (
		idStr.length == (gs.beginningOfGridItemIdStr + 'A').length &&
